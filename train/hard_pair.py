...
output_dir = './npy_data_hard'               # 输出 .npy 文件夹
type = 'hori'                     # 数据集类型
...
def generate_positive_pairs(tiles):
    """
    给定9个图块, 生成 (left, right, label) 的拼图对。
    label=1 为真实左右相邻对, 不需要内部负样本
    """
    pairs = []
    # 创建一个3x3的索引矩阵，方便定位
    idx_matrix = np.arange(9).reshape(3, 3)
...
def load_all_images_and_generate_dataset():
    ...
    # 按照 80% 训练集, 10% 验证集, 10% 测试集的比例划分
    train_plus_valid_files, test_files = train_test_split(all_files, test_size=0.1, random_state=SEED)
    # 再从剩余的90%中分出训练集和验证集 (验证集占原总量的10%，所以test_size=1/9)
    train_files, valid_files = train_test_split(train_plus_valid_files, test_size=1/9, random_state=SEED)

    print(f"数据集划分完毕：{len(train_files)}个训练文件, {len(valid_files)}个验证文件, {len(test_files)}个测试文件。")
    ...
    def process_files_to_dataset(files, description):
        ...
        for filename in tqdm(files, desc=description):
            ...
            positive_pairs = generate_positive_pairs(tiles)
            ...
            # 生成18个外部负样本对
            # 1. 随机选取三张其他图片
            other_files = random.sample(other_files, 3)
            for other_filename in other_files:
                ...
                for r in range(3):  # 遍历所有三行
                    for c in range(2):  # 遍历前两列
                        ...
                        idx2 = idx_matrix[r, c + 1]
                        # 添加 (左边图块, 右边图块, 标签0)
                        all_pairs.append((tiles[idx1], other_tiles[idx2], 0))
    ...
    test_imgs, y_test = process_files_to_dataset(test_files, "生成测试数据")
    ...
    # 测试集
    np.save(os.path.join(output_dir, f'{type}_{category}_test_img_gap_4.npy'), test_imgs)
    np.save(os.path.join(output_dir, f'{type}_{category}_test_label_gap_4.npy'), y_test)
    ...
    print(f"测试集形状: {test_imgs.shape}, 标签数: {len(y_test)}")
...
if __name__ == "__main__":
    type = 'hori'
    image_dir = './hard_pairs/artifacts2'  # 原始图像文件夹
    category = 'artifacts2'
