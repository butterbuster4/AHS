...
def extract_tiles(image, tile_per_side, tile_size):
    """
    按照 JPwLEG 的方式切图：
    ...
    """
    h, w = image.shape[:2]
    expected_size = tile_per_side * tile_size
...
def process_files_to_dataset(files, description, save_prefix):
    ...
    for idx, filename in enumerate(tqdm(files, desc=description)):
        ...
        # --- 内部 pairs ---
        tiles = extract_tiles(img_resized, TILE_PER_SIDE, TILE_SIZE)
        ...
        while generated_outsider_pairs_count < OUTER_NEG and len(other_files) > 0:
            ...
            other_tiles = extract_tiles(other_img_resized, TILE_PER_SIDE, TILE_SIZE)
...
def merge_npz(file_list, save_prefix):
    ...
    np.save(os.path.join(OUTPUT_DIR, f'{save_prefix}_img_gap_48_33.npy'), all_images)
    np.save(os.path.join(OUTPUT_DIR, f'{save_prefix}_label_gap_48_33.npy'), all_labels)
...
if __name__ == "__main__":
    IMAGE_DIR = './JPLEG-3/train'
    TYPE = 'vrti'
    CATEGORY = 'train'
    load_all_images_and_generate_dataset()

    IMAGE_DIR = './JPLEG-3/valid'
    TYPE = 'hori'
    CATEGORY = 'vrti'
    load_all_images_and_generate_dataset()
